...
import { Download, CircleAlert, CircleCheck, CircleX, FileText, Sheet, Database, Wallet, Users, CreditCard, Star } from 'lucide-react';
...
export default function Reports({ appState, setAppState, setActiveTab }) {
  ...
  const redirectToDownload = (blob, fileName) => {
    const url = URL.createObjectURL(blob);
    router.push(`/download?fileUrl=${encodeURIComponent(url)}&fileName=${encodeURIComponent(fileName)}`);
  };
  ...
  const generateCsv = (headers, rows, fileName) => {
    ...
    redirectToDownload(blob, fullFileName);
  };
  ...
  // Full JSON export removed; use Settings → Data Management recommended export
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-center space-y-3 py-10">
              ...
              <p className="text-slate-300">Reports are available on Professional or Enterprise plans.</p>
              <Button onClick={() => setActiveTab && setActiveTab('upgrade')} className="btn-gradient-glow">Upgrade to Unlock</Button>
            </div>
          </CardContent>
        </Card>
      )}
      {isPro && (<div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
        <h2 className="text-2xl md:text-3xl font-bold font-headline">Reports & Data Export</h2>
        <div className="flex gap-2 w-full md:w-auto">
          <Button onClick={() => setActiveTab && setActiveTab('settings')} className="btn-gradient-glow" title="Recommended: A full backup including your settings from the Data Management section.">
              <Star className="mr-2 h-4 w-4" />
              <span className="text-xs md:text-sm">Recommended Export</span>
          </Button>
           <DropdownMenu>
